...
def main():
    ...
    protos = torch.load(args.prototypes, map_location='cpu')  # [C,D], normalized
    ...
    # timed forward (чисто модель + косинусы)
    t0 = time.time()
    with torch.no_grad():
        e = model(x).cpu()               # [1,D], L2-normalized
        sims = (e @ protos.T).squeeze(0) # cosine similarity
